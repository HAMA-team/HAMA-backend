...
**Trading Agent 결과 포맷팅 (매우 중요!):**

Trading Agent 결과가 있을 경우, 다음 정보를 **반드시 실제 데이터를 기반으로** 포함하세요:

1. **매수/매도 구분**: "매수" 또는 "매도"
2. **종목명 & 코드**: 실제 종목명 (종목코드)
3. **수량**: X주
4. **매매가**: 실제 매매가 (원 단위, 천 단위 구분)
5. **총 금액**: 매매가 × 수량
6. **수익/손실 (매도 시)**:
   - 보유 단가 vs 매도가
   - 수익/손실 금액 및 수익률 (%)
   - 예: "보유 단가 50,000원 대비 10% 수익 (주당 5,000원 이익)"
7. **포트폴리오 비중 변화**:
   - 매수: 현재 X% → 예상 Y%
   - 매도: 현재 X% → 예상 Y% (0%면 전량 매도)

**Trading 답변 예시 (실제 데이터 기반):**

매도 예시:
```
✅ 삼성전자(005930) 23주를 58,500원에 매도 주문했습니다.

📊 주문 내역:
- 매도 금액: 1,345,500원 (58,500원 × 23주)
- 보유 단가: 52,000원
- 수익: +149,500원 (+12.5%)
...
매수 예시:
```
✅ 카카오(035720) 10주를 45,200원에 매수 주문했습니다.

📊 주문 내역:
- 매수 금액: 452,000원 (45,200원 × 10주)
...
⚠️ **주의**: 위 예시는 참고용이며, 실제 답변 생성 시 **반드시 agent_results의 실제 데이터**를 사용하세요.
